		...
		<h3><?php echo JText::_('J2STORE_RELATED_PRODUCTS_UPSELLS'); ?></h3>
				<?php foreach($this->up_sells as $upsell_product):?>
					<?php
                        ...
                        $upsel_product_name = $this->escape($upsell_product->product_name);
					?>
		      				...
		      				?>
			   				<?php if(isset($thumb_image) &&  JFile::exists(JPATH::clean(JPATH_SITE.'/'.$thumb_image))):?>
			   					<a href="<?php echo $upsell_product->product_link; ?>">
                                    <img title="<?php echo $upsel_product_name ;?>" alt="<?php echo $upsel_product_name ;?>" class="j2store-product-thumb-image-<?php echo $upsell_product->j2store_product_id; ?>"  src="<?php echo JUri::root().JPath::clean($thumb_image);?>" width="<?php echo intval($upsell_image_width);?>"/>
                                </a>
                            <?php endif; ?>
